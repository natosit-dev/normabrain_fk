...
def ihmt_reg_to_first_acq_mp2rage(wildcards):
    ...
    return expand("data/derivatives/{field_strength}/ihmt/sub-{subject}/ses-{session}/acq-{ihmt_params}/sub-{subject}_ses-{session}_acq-{ihmt_params}_IHMTregisteredto{mp2rage_params}_0GenericAffine.mat", mp2rage_params=first_acq, allow_missing=True)
...
rule register_ihmt_to_MP2RAGE_ants:
    ...
    params:
        outprefix="data/derivatives/{field_strength}/ihmt/sub-{subject}/ses-{session}/acq-{ihmt_params}/sub-{subject}_ses-{session}_acq-{ihmt_params}_IHMTregisteredto{mp2rage_params}_"
    output:
        "data/derivatives/{field_strength}/ihmt/sub-{subject}/ses-{session}/acq-{ihmt_params}/sub-{subject}_ses-{session}_acq-{ihmt_params}_IHMTregisteredto{mp2rage_params}_0GenericAffine.mat"
    conda:
        "../envs/qMT.yaml"
    resources: 
        mem_mb=700
    threads: 4
    log:
       "logs/{field_strength}/ihmt/sub-{subject}/ses-{session}/acq-{ihmt_params}/sub-{subject}_ses-{session}_acq-{ihmt_params}_IHMTregisteredto{mp2rage_params}.log" 
    shell:
        """
...
rule apply_reg_ihmt_to_MP2RAGE_ants:
    input:
        ref="data/derivatives/{field_strength}/MP2RAGE/sub-{subject}/ses-{session}/acq-{mp2rage_params}/t1wUNI_DEN_dicomUnit.nii.gz",
        reg="data/derivatives/{field_strength}/ihmt/sub-{subject}/ses-{session}/acq-{ihmt_params}/sub-{subject}_ses-{session}_acq-{ihmt_params}_IHMTregisteredto{mp2rage_params}_0GenericAffine.mat"
    params:
    ...
    shell:
        ...
        for map in "${{MTmaps[@]}}"; do
            ...
            out="{params.sessiondir}/registered_to_MP2RAGE_ants/{params.subject}_"$map"_registeredtoMP2RAGE{wildcards.mp2rage_params}.nii.gz"
            if [ -f $moving ]; then
                antsApplyTransforms \
                --dimensionality 3 \
...
rule apply_reg_MP2RAGE_to_ihmt_ants:
    ...
    shell:
        ...
        for map in "${{MP2RAGEmaps[@]}}"; do
            ...
            out="{params.mp2rage_acqdir}/registered_to_ihmt_ants/"$map"_registeredto{wildcards.ihmt_params}.nii.gz"
...
rule apply_reg_seg_to_mpm_ants:
    ...
    conda:
        "../envs/qMT.yaml"
    log:
        "logs/{field_strength}/freesurfer/sub-{subject}_ses-{session}_acq-{seq}{mpm_params}/aparc+aseg_registeredtoMPM.log"
    shell:
        """ 
        exec > >(tee {log}) 2>&1 #save output to log AND print to console
...
rule apply_reg_MP2RAGE_to_MPM_ants:
    ...
    params:
        mp2rage_acqdir="data/derivatives/{field_strength}/MP2RAGE/sub-{subject}/ses-{session}/acq-{mp2rage_params}/",
        regto="registeredto{seq}{mpm_params}"
    output:
...
rule register_DWI_to_MP2RAGE_bbregister:
    ...
    output:
        "data/derivatives/{field_strength}/dwi/sub-{subject}/ses-{session}/sub-{subject}_ses-{session}_acq-{dwi_params}_DWIregisteredto{mp2rage_params}/sub-{subject}_ses-{session}_acq-{dwi_params}_DWIregisteredto{mp2rage_params}.lta"
    params:
        subjects_dir="data/derivatives/{field_strength}/freesurfer/",
        subject="sub-{subject}_ses-{session}_acq-{mp2rage_params}",
        outbase="data/derivatives/{field_strength}/dwi/sub-{subject}/ses-{session}/sub-{subject}_ses-{session}_acq-{dwi_params}_DWIregisteredto{mp2rage_params}/sub-{subject}_ses-{session}_acq-{dwi_params}_DWIregisteredto{mp2rage_params}"
    resources:
        mem_mb=1500
    container:
        "docker://freesurfer/freesurfer:8.1.0"
    log:
        "logs/{field_strength}/dwi/sub-{subject}/ses-{session}/sub-{subject}_ses-{session}_acq-{dwi_params}_DWIregisteredto{mp2rage_params}.log"
    shell:
        """
        export SUBJECTS_DIR=$HOME/{params.subjects_dir}
...
rule apply_reg_DWI_to_MP2RAGE_bbregister:
    ...
    params:
        ...
        regto="DWIregisteredto{mp2rage_params}",
