...
rule rician_bias_corr:
    ...
    output:
        denoised="data/derivatives/{field_strength}/MPM/{subject}/{session}/preproc/{subject}_{session}_acq-{seq}{contrast}_mt-{mt}_part-complex_echos4d_riciancorr.nii",
...
rule sos:
    input:
        "data/derivatives/{field_strength}/MPM/{subject}/{session}/preproc/{subject}_{session}_acq-{seq}{contrast}_mt-{mt}_part-{part}_echos4d_denoise_clipped.nii"
    output:
...
rule apply_reg_MPM_to_MP2RAGE_ants:
    ...
    output:
        "data/derivatives/{field_strength}/MPM/{subject}/{session}/{subject}_{session}_acq-{seq}_apply_reg_MPM_to_MP2RAGE_ants.done"
    resources: 
        mem_mb=500
    conda:
        "../envs/qMT.yaml"
